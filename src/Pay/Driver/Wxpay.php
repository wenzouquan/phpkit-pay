...
require_once dirname(__FILE__) . '/WxPayPubHelper/lib/WxPay.Api.php';
require_once dirname(__FILE__) . '/WxPayPubHelper/lib/WxPay.NativePay.php';
...
class Wxpay extends \phpkit\pay\Pay\Pay {
	...
	public function check() {
		/*$this->alipay_config['APPID'] = 'wxd8e7a93dd0cb2c14';
			        $this->alipay_config['MCHID']  = '10036811';
			        $this->alipay_config['KEY']  = '192006250b4c09247ec02edce69f6a2c';
		*/
		//$this->alipay_config['notify_url']='http://mall.ailvo.com/Public/notify/method/notify';
	...
	public function buildRequestForm(\phpkit\pay\Pay\PayVo $vo) {
		$input = new WxPayPubHelper\lib\WxPayUnifiedOrder();
		$notify = new WxPayPubHelper\lib\NativePay();
	...
	public function verifyNotify($notifydata) {
		...
		echo $returnXml;
		//BoxModel("test")->add(array('test'=>$returnXml));//测试可以删除
		//BoxModel("test")->add(array('test'=>"verifyNotify"));//测试可以删除
		//==商户根据实际情况设置相应的处理流程，此处仅作举例=======
		//以log文件形式记录回调信息
		if ($notify->checkSign() == TRUE) {
			// BoxModel("test")->add(array('test'=>"签名成功了"));//测试可以删除
			if ($notify->data["return_code"] == "FAIL") {
				//此处应该更新一下订单状态，商户自行增删操作
				//$log_->log_result($log_name,"【通信出错】:\n".$xml."\n");
				//BoxModel("test")->add(array('test'=>"【通信出错】:\n".$xml."\n"));//测试可以删除作
				return false;
			} elseif ($notify->data["result_code"] == "FAIL") {
				//此处应该更新一下订单状态，商户自行增删操作
				//$log_->log_result($log_name,"【业务出错】:\n".$xml."\n");
				//BoxModel("test")->add(array('test'=>"【业务出错】:\n".$xml."\n"));//测试可以删除作
				return false;
			} else {
				//此处应该更新一下订单状态，商户自行增删操
				//	BoxModel("test")->add(array('test'=>"支付成功"));//测试可以删除作
...
class UnifiedOrder_pub extends Wxpay_client_pub {
	function __construct() {
		...
		$this->url = "http://182.254.158.200/curlTenpay.php";
		//设置curl超时时间
